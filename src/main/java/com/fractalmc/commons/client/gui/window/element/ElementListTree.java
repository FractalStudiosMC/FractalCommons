...
import com.fractalmc.commons.client.gui.window.WindowTopDockBase;
import com.fractalmc.commons.client.render.RendererHelper;
import com.fractalmc.commons.common.core.utils.IOUtils;
        ...
        public Tree draw(int mouseX, int mouseY, boolean hover, int width, int treeHeight, boolean hasScroll, int totalHeight, boolean clicking, boolean rClicking)
        {
            if(!(treeDragged == this && !(dragX == mouseX && dragY == mouseY)) || dragDraw)
            {
                ...
                if(attachedObject instanceof File)
                {
                    ...
                    parent.workspace.getFontRenderer().drawString(IOUtils.readableFileSize(info.length()), getPosX() + offX + width - 4 - parent.workspace.getFontRenderer().getStringWidth(IOUtils.readableFileSize(info.length())), getPosY() + offY + 3 + treeHeight, Theme.getAsHex(parent.workspace.currentTheme.font), false);
                }
