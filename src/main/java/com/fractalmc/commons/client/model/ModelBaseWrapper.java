package com.fractalmc.commons.client.model;
...
public class ModelBaseWrapper implements IBakedModel
{
    ...
    public List<BakedQuad> getQuads(@Nullable IBlockState state, @Nullable EnumFacing side, long rand)
    {
        ...
        if(!disableRender)
        {
            ...
            modelBase.renderModel(FractalCommons.eventHandlerClient.renderTick);
    ...
    public Pair<? extends IBakedModel, Matrix4f> handlePerspective(ItemCameraTransforms.TransformType cameraTransformType)
    {
        ...
        if(modelBase.useVanillaCameraTransform())
        {
            ...
            {
                float prog = (float)Math.sin(MathHelper.clamp((isLeft ? EntityHelper.interpolateValues(ItemRenderingHelper.prevDualHandedAnimationLeft, ItemRenderingHelper.dualHandedAnimationLeft, FractalCommons.eventHandlerClient.renderTick) : EntityHelper.interpolateValues(ItemRenderingHelper.prevDualHandedAnimationRight, ItemRenderingHelper.dualHandedAnimationRight, FractalCommons.eventHandlerClient.renderTick)) / (float)ItemRenderingHelper.dualHandedAnimationTime, 0F, 1F) * Math.PI / 4F);
