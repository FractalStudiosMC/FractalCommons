...
import com.fractalmc.commons.client.gui.window.element.IIdentifiable;
import com.fractalmc.commons.client.gui.window.element.IListable;
...
import com.fractalmc.commons.common.core.config.annotations.StringValues;
import com.fractalmc.commons.common.core.config.types.Colour;
import com.fractalmc.commons.common.core.config.types.NestedIntArray;
...
public abstract class ConfigBase implements Comparable<ConfigBase>, IIdentifiable, IListable
{
    ...
    public void readProperty(Field field, boolean write)
    {
        ...
        {
            FractalCommons.internalLogger.logWarn("Config property " + field.getName() + " from mod " + getModName() + " may not be localized!");
        }
        ...
        try
        {
            ...
            else if(clz.equals(KeyBind.class))
            {
                ...
                else
                {
                    ...
                    bind = FractalCommons.proxy.registerKeyBind(bind, null);
                    field.set(this, bind);
                }
            }
            else
    ...
    public void setCategoryComments()
    {
        for(Map.Entry<CategoryInfo, ArrayList<ConfigBase.PropInfo>> e : categories.entrySet())
        {
            ...
            {
                info.name = I18n.translateToLocal(String.format("fractalcommons.config.cat.%s.name", cat));
                comment = I18n.translateToLocal(String.format("fractalcommons.config.cat.%s.comment", cat));
            }
            else
            {
    ...
    {
        FractalCommons.channel.sendTo(new PacketSession(this), player);
    }

    public void updateSessionToAllPlayers()
    {
        FractalCommons.channel.sendToAll(new PacketSession(this));
    }
