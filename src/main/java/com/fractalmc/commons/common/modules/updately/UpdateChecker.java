package com.fractalmc.commons.common.modules.updately;
...
public class UpdateChecker
{
        ...
        for(ModVersionInfo info : registeredMods)
        {
            ...
            if(versionInfo != null && versionInfo.containsKey(info.mcVersion))
            {
                ...
                if(diff > 0)
                {
                    ...
                    {
                        FractalCommons.internalLogger.logWarn("[NEW UPDATE AVAILABLE] " + info.modName + " - " + info.modVersionNew);
                    }
                }
            }
        }
    ...
    public static void serverStarted()
    {
        ...
        {
            UpdateChecker.getModsWithUpdates().stream().filter(info -> !info.isModClientOnly).forEach(info -> FractalCommons.internalLogger.logWarn("[NEW UPDATE AVAILABLE] " + info.modName + " - " + info.modVersionNew));
        }
        else
        {
            requireLogging = true;
